...
DEVICE_PATH := device/intel/baytrail/bigcore

TARGET_USE_DROIDBOOT := true

include device/intel/baytrail/bigcore/PlatformBoardConfig.mk
...
BOARD_KERNEL_CMDLINE := console=ttyS0,115200 console=ttyUSB0,115200 console=logk0 earlyprintk loglevel=8 drm.debug=0xe kmemleak=off ptrace.ptrace_can_access=1 androidboot.bootmedia=$(BOARD_BOOTMEDIA) androidboot.hardware=$(TARGET_DEVICE) $(cmdline_extra) nmi_watchdog=panic softlockup_panic=1 vmalloc=172M
else ifeq ($(TARGET_BUILD_VARIANT),userdebug)
BOARD_KERNEL_CMDLINE := console=ttyS0,115200 console=ttyUSB0,115200 console=logk0 earlyprintk loglevel=4 kmemleak=off ptrace.ptrace_can_access=1 androidboot.bootmedia=$(BOARD_BOOTMEDIA) androidboot.hardware=$(TARGET_DEVICE) $(cmdline_extra) nmi_watchdog=panic softlockup_panic=1 vmalloc=172M
else
